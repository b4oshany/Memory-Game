...
var userdata = {"username": null, "turn": 0, "photo": null};
...
if(typeof(Storage) !== undefined){
    ...
    if(localStorage.gamer === undefined)
        saveData(userdata);
    else{
...
var turn = (supportStorage && !isNaN(userdata.turn))? parseInt(userdata.turn): 0;
...
function F(t) {
    ...
    if (x && !y && x != t && x.innerHTML == t.innerHTML) C(x, "p"), C(t, "p"), l--;
        ...
        C(t, "v"); (clicks%2==0)?turn++:'';
        if(supportStorage) userdata.turn = turn;
        saveData(userdata);
        userturn.innerHTML = turn; 
    }
    S(true);
